...
opt.out = "./output/" # Where to output data
opt.entity = "listerine"           # Name of entity to output
opt.data = "./models/Artec/Artec/listerine"
opt.model = f"{opt.data}/listerine.obj" # Path to object file
...
def f(frame_ids):
    ...
    negatives = list(glob.glob("negatives/*.png"))
    ...
    texture = visii.texture.create_from_file(opt.entity,f"{opt.data}/{opt.entity}.jpg")
    ...
    for i in tqdm(frame_ids):
        ...
        stretch_factor = 2
        scale = [
            random.uniform(1,stretch_factor) * 5,  # width
            random.uniform(1,stretch_factor) * 5,   # length
            random.uniform(1,stretch_factor) * 5   # height 
        ]
        ...
        rot = [
            random.uniform(-10, 10) + 90, # Roll
            random.uniform(-15, 15), # Pitch
            random.uniform(-90, 90) - 90 # Yaw
        ]
